...
void t_system_script_log_chatter_socket_stream (void) {
	...
	__catch10_g_error:
	{
		...
		g_remove (_tmp80_);
		_tmp81_ = e->message;
		g_error ("test-umockdev-record.vala:642: Error: %s", _tmp81_);
		_g_error_free0 (e);
	}
	__finally10:
	if (_inner_error_ != NULL) {
...
void t_gphoto2_record (void) {
	...
	__catch14_g_regex_error:
	{
		...
		_inner_error_ = NULL;
		_tmp17_ = e->message;
		g_error ("test-umockdev-record.vala:766: Internal error building regex: %s", _tmp17_);
		_g_error_free0 (e);
	}
	__finally14:
	if (_inner_error_ != NULL) {
	...
	if (!_tmp21_) {
		FILE* _tmp22_ = NULL;
		_tmp22_ = stdout;
		fprintf (_tmp22_, "[SKIP: no gphoto2 compatible camera attached] ");
		_g_match_info_unref0 (match);
		_g_regex_unref0 (port_re);
		_g_free0 (serr);
		_g_free0 (sout_record);
	...
	_tmp23_ = match;
	_tmp24_ = g_match_info_fetch (_tmp23_, 1);
	busnum = _tmp24_;
	_tmp25_ = match;
	_tmp26_ = g_match_info_fetch (_tmp25_, 2);
	devnum = _tmp26_;
	_tmp27_ = umockdev_record_path;
	_tmp28_ = busnum;
	_tmp29_ = devnum;
	_tmp30_ = g_strdup_printf ("sh -c '%s /dev/bus/usb/%s/%s > gphoto-test.umockdev'", _tmp27_, _tmp28_, _tmp29_);
	_tmp31_ = _tmp30_;
	spawn (_tmp31_, &_tmp32_, &_tmp33_, &_tmp34_);
	_g_free0 (sout);
	sout = _tmp32_;
	_g_free0 (serr);
	serr = _tmp33_;
	...
	_tmp38_ = g_strdup ("sh -c 'gphoto2 -l; gphoto2 -L'");
	cmd = _tmp38_;
	_tmp39_ = umockdev_record_path;
	_tmp40_ = busnum;
	_tmp41_ = devnum;
	_tmp42_ = cmd;
	_tmp43_ = g_strdup_printf ("%s -i /dev/bus/usb/%s/%s=gphoto-test.ioctl -- %s", _tmp39_, _tmp40_, _tmp41_, _tmp42_);
	_tmp44_ = _tmp43_;
	spawn (_tmp44_, &_tmp45_, &_tmp46_, &_tmp47_);
	_g_free0 (sout_record);
	sout_record = _tmp45_;
	_g_free0 (serr);
	serr = _tmp46_;
	...
	_tmp49_ = exit;
	g_assert_cmpint (_tmp49_, ==, 0);
	_tmp50_ = umockdev_run_path;
	_tmp51_ = busnum;
	_tmp52_ = devnum;
	_tmp53_ = cmd;
	_tmp54_ = g_strdup_printf ("%s -d gphoto-test.umockdev -i /dev/bus/usb/%s/%s=gphoto-test.ioctl -- " \
"%s", _tmp50_, _tmp51_, _tmp52_, _tmp53_);
	_tmp55_ = _tmp54_;
	spawn (_tmp55_, &_tmp56_, &_tmp57_, &_tmp58_);
	_g_free0 (sout);
	sout = _tmp56_;
	...
	_tmp61_ = sout;
	_tmp62_ = sout_record;
	g_assert_cmpstr (_tmp61_, ==, _tmp62_);
