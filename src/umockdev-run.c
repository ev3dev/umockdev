/* umockdev-run.c generated by valac 0.39.91, the Vala compiler
            ...
            int args_length1)
{
	...
	struct sigaction _tmp127_;
	sigset_t _tmp128_;
	struct sigaction _tmp129_;
	gint _tmp130_;
	struct sigaction _tmp131_;
	gint _tmp132_;
	struct sigaction _tmp133_;
	gint _tmp134_;
	struct sigaction _tmp135_;
	gint _tmp136_;
	struct sigaction _tmp137_;
	gint _tmp138_;
	GPid _tmp139_;
	gint _tmp140_ = 0;
	GPid _tmp141_;
	...
	act = _tmp126_;
	_tmp127_ = act;
	_tmp128_ = _tmp127_.sa_mask;
	sigemptyset (&_tmp128_);
	_tmp129_ = act;
	_tmp130_ = sigaction (SIGTERM, &_tmp129_, NULL);
	_vala_assert (_tmp130_ == 0, "Posix.sigaction (Posix.SIGTERM, act, null) == 0");
	_tmp131_ = act;
	_tmp132_ = sigaction (SIGHUP, &_tmp131_, NULL);
	_vala_assert (_tmp132_ == 0, "Posix.sigaction (Posix.SIGHUP, act, null) == 0");
	_tmp133_ = act;
	_tmp134_ = sigaction (SIGINT, &_tmp133_, NULL);
	_vala_assert (_tmp134_ == 0, "Posix.sigaction (Posix.SIGINT, act, null) == 0");
	_tmp135_ = act;
	_tmp136_ = sigaction (SIGQUIT, &_tmp135_, NULL);
	_vala_assert (_tmp136_ == 0, "Posix.sigaction (Posix.SIGQUIT, act, null) == 0");
	_tmp137_ = act;
	_tmp138_ = sigaction (SIGABRT, &_tmp137_, NULL);
	_vala_assert (_tmp138_ == 0, "Posix.sigaction (Posix.SIGABRT, act, null) == 0");
	_tmp139_ = child_pid;
	waitpid ((pid_t) _tmp139_, &_tmp140_, 0);
	status = _tmp140_;
	_tmp141_ = child_pid;
	g_spawn_close_pid (_tmp141_);
	_g_object_unref0 (testbed);
	testbed = NULL;
	_tmp142_ = status;
	if (WIFEXITED (_tmp142_)) {
		gint _tmp143_;
		_tmp143_ = status;
		result = WEXITSTATUS (_tmp143_);
		_g_object_unref0 (testbed);
		_g_free0 (preload);
		_g_option_context_free0 (oc);
		return result;
	...
	_tmp144_ = status;
	if (WIFSIGNALED (_tmp144_)) {
		gint _tmp145_;
		_tmp145_ = status;
		raise (WTERMSIG (_tmp145_));
	}
	result = status;
	_g_object_unref0 (testbed);
	_g_free0 (preload);
