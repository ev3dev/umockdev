/* umockdev-record.c generated by valac 0.24.0, the Vala compiler
...
void record_device (const gchar* dev) {
		...
		for (line_it = 0; line_it < _vala_array_length (_tmp20_); line_it = line_it + 1) {
			...
			line = _tmp22_;
			{
				...
				if (_tmp26_) {
					...
					_tmp32_ = line;
					_tmp33_ = g_str_has_prefix (_tmp32_, "E: UDEV_LOG=");
					_tmp25_ = _tmp33_;
				}
				if (_tmp25_) {
					_tmp24_ = TRUE;
					...
					gboolean _tmp35_ = FALSE;
					_tmp34_ = line;
					_tmp35_ = g_str_has_prefix (_tmp34_, "E: USEC_INITIALIZED=");
					_tmp24_ = _tmp35_;
				}
				if (_tmp24_) {
					_tmp23_ = TRUE;
					...
					_tmp36_ = line;
					_tmp37_ = g_str_has_prefix (_tmp36_, "L: ");
					_tmp23_ = _tmp37_;
				}
				if (_tmp23_) {
					_g_free0 (line);
				...
				_tmp38_ = line;
				_tmp39_ = g_str_has_prefix (_tmp38_, "N: ");
				if (_tmp39_) {
					...
					_tmp40_ = line;
					_tmp41_ = line;
					_tmp42_ = string_substring (_tmp41_, (glong) 3, (glong) (-1));
					_tmp43_ = _tmp42_;
					_tmp44_ = string_chomp (_tmp43_);
					_tmp45_ = _tmp44_;
					_tmp46_ = g_strconcat ("/dev/", _tmp45_, NULL);
					_tmp47_ = _tmp46_;
					_tmp48_ = dev_contents (_tmp47_);
					_tmp49_ = _tmp48_;
					_tmp50_ = g_strconcat (_tmp40_, _tmp49_, NULL);
					_g_free0 (line);
					line = _tmp50_;
					_g_free0 (_tmp49_);
					_g_free0 (_tmp47_);
	...
	_tmp54_ = dev;
	_tmp55_ = string_contains (_tmp54_, "tegra");
	if (_tmp55_) {
		FILE* _tmp56_ = NULL;
		_tmp56_ = stdout;
		fputc ('\n', _tmp56_);
...
void split_devfile_arg (const gchar* arg, gchar** dev, gchar** devnum, gchar** fname) {
	...
	if (_tmp16_) {
		...
		_tmp28_ = minor (_tmp27_);
		_tmp29_ = g_strdup_printf ("%u", (_tmp25_ << 8) + _tmp28_);
		_g_free0 (_vala_devnum);
		_vala_devnum = _tmp29_;
	} else {
		struct stat _tmp30_ = {0};
		mode_t _tmp31_ = {0};
		gboolean _tmp32_ = FALSE;
		_tmp30_ = st;
		_tmp31_ = _tmp30_.st_mode;
		_tmp32_ = S_ISSOCK (_tmp31_);
		if (_tmp32_) {
			...
			_g_free0 (_tmp44_);
			fields = _tmp47_;
			fields_length1 = _tmp47__length1;
			_fields_size_ = fields_length1;
			_tmp48_ = fields;
			_tmp48__length1 = fields_length1;
			_vala_assert (_tmp48__length1 == 2, "fields.length == 2");
			_tmp49_ = fields;
			_tmp49__length1 = fields_length1;
			_tmp50_ = _tmp49_[0];
			_tmp51_ = atoi (_tmp50_);
			_tmp52_ = fields;
			_tmp52__length1 = fields_length1;
			_tmp53_ = _tmp52_[1];
			_tmp54_ = atoi (_tmp53_);
			_tmp55_ = g_strdup_printf ("%i", (_tmp51_ << 8) | _tmp54_);
			_g_free0 (_vala_devnum);
			_vala_devnum = _tmp55_;
