/* umockdev.c generated by valac 0.39.91, the Vala compiler
                                         ...
                                         GError** error)
{
	...
	ptym = _tmp7_;
	ptys = _tmp8_;
	_vala_assert (_tmp9_ == 0, "Linux.openpty (out ptym, out ptys, ptyname_array, null, null) == 0");
...
"t pty %s", node_path, _tmp12_);
	...
	close (_tmp13_);
	_tmp14_ = ptym;
	_tmp16_ = tcgetattr (_tmp14_, &_tmp15_);
	ios = _tmp15_;
	_vala_assert (_tmp16_ == 0, "Posix.tcgetattr (ptym, out ios) == 0");
	_tmp17_ = ios.c_iflag;
	ios.c_iflag = _tmp17_ & (~((IGNCR | INLCR) | ICRNL));
	_tmp18_ = ios.c_oflag;
	ios.c_oflag = _tmp18_ & (~(ONLCR | OCRNL));
	_tmp19_ = ios.c_lflag;
	ios.c_lflag = _tmp19_ & (~(ICANON | ECHO));
	cfmakeraw (&ios);
	_tmp20_ = ptym;
	_tmp21_ = ios;
	_vala_assert (tcsetattr (_tmp20_, TCSANOW, &_tmp21_) == 0, "Posix.tcsetattr (ptym, Posix.TCSANOW, ios) == 0");
	_tmp22_ = ptyname;
	_vala_assert (symlink (_tmp22_, node_path) == 0, "FileUtils.symlink (ptyname, node_path) == 0");
	if (majmin != NULL) {
		gchar* mapdir = NULL;
		const gchar* _tmp23_;
		gchar* _tmp24_;
		...
		const gchar* _tmp32_;
		const gchar* _tmp33_;
		_tmp23_ = self->priv->root_dir;
		_tmp24_ = g_build_filename (_tmp23_, "dev", ".ptymap", NULL);
		mapdir = _tmp24_;
		_tmp25_ = mapdir;
		g_mkdir_with_parents (_tmp25_, 0755);
		_tmp26_ = mapdir;
		_tmp27_ = ptyname;
		...
		dest = _tmp31_;
		_tmp32_ = dest;
		g_debug ("umockdev.vala:1104: create_node_for_device: creating ptymap symlink %s", _tmp32_);
		_tmp33_ = dest;
		_vala_assert (symlink (majmin, _tmp33_) == 0, "FileUtils.symlink(majmin, dest) == 0");
		_g_free0 (dest);
		_g_free0 (mapdir);
	}
	...
	_tmp36_ = _tmp35_;
	_tmp37_ = string_substring (node_path, (glong) _tmp36_, (glong) -1);
	devname = _tmp37_;
	_tmp38_ = self->priv->dev_fd;
	_tmp39_ = devname;
	_vala_assert (!g_hash_table_contains (_tmp38_, _tmp39_), "!this.dev_fd.contains (devname)");
	_tmp40_ = self->priv->dev_fd;
	_tmp41_ = devname;
	_tmp42_ = g_strdup (_tmp41_);
	_tmp43_ = ptym;
	g_hash_table_insert (_tmp40_, _tmp42_, (gpointer) ((gintptr) _tmp43_));
	_g_free0 (devname);
	_g_free0 (ptyname);
                     ...
                     gboolean remove_toplevel)
{
	...
	if (_tmp0_) {
		...
		__catch10_g_file_error:
		{
			...
			g_warning ("umockdev.vala:1251: cannot open: %s: %s", path, _tmp5_);
			_g_error_free0 (e);
			_g_dir_close0 (d);
			return;
		}
	...
	if (remove_toplevel) {
		if (g_remove (path) < 0) {
			...
			g_warning ("umockdev.vala:1262: cannot remove %s: %s", path, _tmp14_);
		}
	}
}
...
"a '%s'", _tmp10_, _tmp13_);
				...
				if (_tmp17_ < ((gssize) 0)) {
					...
					g_error ("umockdev.vala:1385: ScriptRunner[%s]: write failed: %s", _tmp18_, _tmp20_);
				}
				_tmp21_ = l;
				_tmp22_ = data;
			...
			case 'w':
			{
				...
				_tmp26_ = _tmp25_;
				g_debug ("umockdev.vala:1390: ScriptRunner[%s]: write op, data '%s'", _tmp23_, _tmp26_);
				_g_free0 (_tmp26_);
				_tmp27_ = data;
...
"e between 0 and 100)", _tmp30_, (guint) _tmp31_);
				...
				g_debug ("umockdev.vala:1403: ScriptRunner[%s]: setting fuzz level to %u%%", _tmp33_, _tmp34_);
				break;
			}
			case 'd':
			{
			...
			default:
			{
				...
				_tmp36_ = op;
				g_debug ("umockdev.vala:1411: ScriptRunner[%s]: got unknown line op %c, ignoring", _tmp35_, (gint) _tmp36_);
				break;
			}
		}
	}
	...
	g_return_val_if_fail (self != NULL, NULL);
	{
		...
		while (TRUE) {
			...
			if (_tmp2_ == EOF) {
				...
				g_debug ("umockdev.vala:1427: ScriptRunner[%s]: end of script %s, closing", _tmp3_, _tmp4_);
				_vala_op = 'Q';
...
"on %li", _tmp15_, _tmp16_);
	...
	if (fscanf (_tmp18_, "%" G_GUINT32_FORMAT " ", &_vala_delta) != 1) {
		...
		_tmp20_ = cur_pos;
		g_error ("umockdev.vala:1446: Cannot parse time in %s at position %li", _tmp19_, _tmp20_);
	}
                                 ...
                                 guint32 delta)
{
	...
	while (TRUE) {
		...
		if (_tmp9_ < 0) {
			...
			g_error ("umockdev.vala:1471: ScriptRunner op_write[%s]: select() failed: %s", _tmp11_, _tmp13_);
		}
		_tmp14_ = res;
		if (_tmp14_ == 0) {
			const gchar* _tmp15_;
                            ...
                            const gchar* record_file)
{
	...
	__catch12_g_error:
	{
		...
		_inner_error_ = NULL;
		_tmp12_ = e->message;
		g_error ("umockdev.vala:1626: load_socket_script(): cannot create Socket: %s", _tmp12_);
		_g_error_free0 (e);
	}
	__finally12:
	if (G_UNLIKELY (_inner_error_ != NULL)) {
	...
	g_debug ("umockdev.vala:1629: SocketServer.add: Created socket path %s, fd %i", sock_path, fd);
...
umockdev_socket_server_run (UMockdevSocketServer* self)
{
	...
	while (TRUE) {
		...
		if (_tmp17_ < 0) {
			...
			_tmp19_ = errno;
			_tmp20_ = g_strerror (_tmp19_);
			g_error ("umockdev.vala:1659: socket server thread: select() failed: %s", _tmp20_);
		}
		_tmp21_ = res;
		if (_tmp21_ == 0) {
			continue;
				...
				s = s_collection[s_it];
				{
					...
					if (FD_ISSET (_tmp28_, &_tmp29_) > 0) {
						...
						if (_tmp33_ < 0) {
							...
							g_error ("umockdev.vala:1679: socket server thread: accept() failed: %s", _tmp35_);
						}
						sock_path = NULL;
						{
