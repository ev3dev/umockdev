/* umockdev.c generated by valac 0.30.1, the Vala compiler
...
" /sys/", _tmp3_);
	...
	_tmp34_ = subsystem;
	if (g_strcmp0 (_tmp34_, "usb") != 0) {
		const gchar* _tmp35_ = NULL;
		_tmp35_ = subsystem;
		_tmp33_ = g_strcmp0 (_tmp35_, "pci") != 0;
	} else {
		_tmp33_ = FALSE;
	}
	if (_tmp33_) {
		...
		_tmp36_ = subsystem;
		_tmp32_ = g_strcmp0 (_tmp36_, "thunderbolt") != 0;
	} else {
		_tmp32_ = FALSE;
	}
	if (_tmp32_) {
		...
		gchar* _tmp63_ = NULL;
		gint _tmp64_ = 0;
		_tmp37_ = self->priv->sys_dir;
		_tmp38_ = subsystem;
		_tmp39_ = g_build_filename (_tmp37_, "class", _tmp38_, NULL);
		class_dir = _tmp39_;
		_tmp40_ = class_dir;
		_tmp41_ = g_mkdir_with_parents (_tmp40_, 0755);
		if (_tmp41_ != 0) {
			...
			g_error ("umockdev.vala:412: cannot create class dir '%s': %s", _tmp42_, _tmp44_);
		}
		_tmp45_ = dev_path;
		_tmp46_ = umockdev_make_dotdots (_tmp45_);
		_tmp47_ = _tmp46_;
		_tmp48_ = subsystem;
		_tmp49_ = g_build_filename (_tmp47_, "class", _tmp48_, NULL);
		_tmp50_ = _tmp49_;
		_tmp51_ = dev_dir;
		_tmp52_ = g_build_filename (_tmp51_, "subsystem", NULL);
		_tmp53_ = _tmp52_;
		_tmp54_ = symlink (_tmp50_, _tmp53_);
		_vala_assert (_tmp54_ == 0, "FileUtils.symlink(Path.build_filename(make_dotdots(dev_path), \"class\", subsystem),                                      Path.build_filename(dev_dir, \"subsystem\")) == 0");
		_g_free0 (_tmp53_);
		_g_free0 (_tmp50_);
		_g_free0 (_tmp47_);
		_tmp55_ = dev_path_no_sys;
		_tmp56_ = g_build_filename ("..", "..", _tmp55_, NULL);
		_tmp57_ = _tmp56_;
		_tmp58_ = class_dir;
		_tmp59_ = name;
		_tmp60_ = g_path_get_basename (_tmp59_);
		_tmp61_ = _tmp60_;
		_tmp62_ = g_build_filename (_tmp58_, _tmp61_, NULL);
		_tmp63_ = _tmp62_;
		_tmp64_ = symlink (_tmp57_, _tmp63_);
		_vala_assert (_tmp64_ == 0, "FileUtils.symlink(Path.build_filename(\"..\", \"..\", dev_path_no_sys),                                      Path.build_filename(class_dir, Path.get_basename(name))) == 0");
		_g_free0 (_tmp63_);
		_g_free0 (_tmp61_);
		_g_free0 (_tmp57_);
		_g_free0 (class_dir);
		...
		_vala_assert (_tmp69_ == 0, "DirUtils.create_with_parents(bus_dir, 0755) == 0");
		...
		_vala_assert (_tmp79_ == 0, "FileUtils.symlink(Path.build_filename(\"..\", \"..\", \"..\", dev_path_no_sys),                                      Path.build_filename(bus_dir, Path.get_basename(name))) == 0");
		_g_free0 (_tmp78_);
		_g_free0 (_tmp76_);
		_g_free0 (_tmp72_);
		_tmp80_ = dev_path;
		_tmp81_ = umockdev_make_dotdots (_tmp80_);
		_tmp82_ = _tmp81_;
		_tmp83_ = subsystem;
		_tmp84_ = g_build_filename (_tmp82_, "bus", _tmp83_, NULL);
		_tmp85_ = _tmp84_;
		_tmp86_ = dev_dir;
		_tmp87_ = g_build_filename (_tmp86_, "subsystem", NULL);
		_tmp88_ = _tmp87_;
		_tmp89_ = symlink (_tmp85_, _tmp88_);
		_vala_assert (_tmp89_ == 0, "FileUtils.symlink(Path.build_filename(make_dotdots(dev_path), \"bus\", subsystem),                                      Path.build_filename(dev_dir, \"subsystem\")) == 0");
		_g_free0 (_tmp88_);
		_g_free0 (_tmp85_);
		_g_free0 (_tmp82_);
		_g_free0 (bus_dir);
	...
	if (g_strcmp0 (_tmp90_, "block") == 0) {
		...
		gchar* _tmp106_ = NULL;
		gint _tmp107_ = 0;
		_tmp91_ = self->priv->sys_dir;
		_tmp92_ = g_build_filename (_tmp91_, "block", NULL);
		block_dir = _tmp92_;
		_tmp93_ = block_dir;
		_tmp94_ = g_mkdir_with_parents (_tmp93_, 0755);
		if (_tmp94_ != 0) {
			...
			_tmp95_ = block_dir;
			_tmp96_ = errno;
			_tmp97_ = g_strerror (_tmp96_);
			g_error ("umockdev.vala:439: cannot create block dir '%s': %s", _tmp95_, _tmp97_);
		}
		_tmp98_ = dev_path_no_sys;
		_tmp99_ = g_build_filename ("..", _tmp98_, NULL);
		_tmp100_ = _tmp99_;
		_tmp101_ = block_dir;
		_tmp102_ = name;
		_tmp103_ = g_path_get_basename (_tmp102_);
		_tmp104_ = _tmp103_;
		_tmp105_ = g_build_filename (_tmp101_, _tmp104_, NULL);
		_tmp106_ = _tmp105_;
		_tmp107_ = symlink (_tmp100_, _tmp106_);
		_vala_assert (_tmp107_ == 0, "FileUtils.symlink(Path.build_filename(\"..\", dev_path_no_sys),                                      Path.build_filename(block_dir, Path.get_basename(name))) == 0");
		_g_free0 (_tmp106_);
		_g_free0 (_tmp104_);
		_g_free0 (_tmp100_);
		_g_free0 (block_dir);
	...
	props = _tmp108_;
	{
		...
		i = 0;
		{
			...
			while (TRUE) {
				...
				if (!(_tmp111_ < (_tmp112__length1 - 1))) {
					break;
				}
				_tmp114_ = properties;
				_tmp114__length1 = _vala_array_length (properties);
				_tmp115_ = i;
				_tmp116_ = _tmp114_[_tmp115_];
				if (g_strcmp0 (_tmp116_, "DEVNAME") == 0) {
					gchar** _tmp117_ = NULL;
					...
					_tmp120_ = g_str_has_prefix (_tmp119_, "/dev/");
					_tmp113_ = _tmp120_;
				} else {
					_tmp113_ = FALSE;
				}
				if (_tmp113_) {
					...
					_tmp125_ = props;
					_tmp126_ = dev_node;
					_tmp127_ = g_strconcat ("DEVNAME=", _tmp126_, NULL);
					_tmp128_ = _tmp127_;
					...
					_tmp132_ = props;
					_tmp133_ = properties;
					_tmp133__length1 = _vala_array_length (properties);
					_tmp134_ = i;
					_tmp135_ = _tmp133_[_tmp134_];
					_tmp136_ = g_strconcat (_tmp135_, "=", NULL);
					_tmp137_ = _tmp136_;
					_tmp138_ = properties;
					_tmp138__length1 = _vala_array_length (properties);
					_tmp139_ = i;
					_tmp140_ = _tmp138_[_tmp139_ + 1];
					_tmp141_ = g_strconcat (_tmp137_, _tmp140_, NULL);
					_tmp142_ = _tmp141_;
					_tmp143_ = g_strconcat (_tmp142_, "\n", NULL);
					_tmp144_ = _tmp143_;
					_tmp145_ = g_strconcat (_tmp132_, _tmp144_, NULL);
					_g_free0 (props);
					props = _tmp145_;
					_g_free0 (_tmp144_);
					_g_free0 (_tmp142_);
	...
	if ((_tmp146__length1 % 2) != 0) {
		...
		_tmp147_ = properties;
		_tmp147__length1 = _vala_array_length (properties);
		_tmp148_ = properties;
		_tmp148__length1 = _vala_array_length (properties);
		_tmp149_ = _tmp147_[_tmp148__length1 - 1];
		g_warning ("umockdev.vala:455: add_devicev: Ignoring property key '%s' without val" \
"ue", _tmp149_);
	}
	_tmp150_ = dev_path;
	_tmp151_ = props;
	umockdev_testbed_set_attribute (self, _tmp150_, "uevent", _tmp151_);
	{
		gint i = 0;
		i = 0;
		{
			...
			while (TRUE) {
				...
				if (!(_tmp154_ < (_tmp155__length1 - 1))) {
					break;
				}
				_tmp156_ = dev_path;
				_tmp157_ = attributes;
				_tmp157__length1 = _vala_array_length (attributes);
				_tmp158_ = i;
				_tmp159_ = _tmp157_[_tmp158_];
				_tmp160_ = attributes;
				_tmp160__length1 = _vala_array_length (attributes);
				_tmp161_ = i;
				_tmp162_ = _tmp160_[_tmp161_ + 1];
				umockdev_testbed_set_attribute (self, _tmp156_, _tmp159_, _tmp162_);
				_tmp164_ = attributes;
				_tmp164__length1 = _vala_array_length (attributes);
				_tmp165_ = i;
				_tmp166_ = _tmp164_[_tmp165_];
				if (g_strcmp0 (_tmp166_, "dev") == 0) {
					const gchar* _tmp167_ = NULL;
					_tmp167_ = dev_node;
					_tmp163_ = _tmp167_ != NULL;
				} else {
					_tmp163_ = FALSE;
				}
				if (_tmp163_) {
					...
					_tmp168_ = self->priv->root_dir;
					_tmp169_ = g_build_filename (_tmp168_, "dev", ".node", NULL);
					infodir = _tmp169_;
					_tmp170_ = infodir;
					g_mkdir_with_parents (_tmp170_, 0755);
					_tmp171_ = attributes;
					_tmp171__length1 = _vala_array_length (attributes);
					_tmp172_ = i;
					_tmp173_ = _tmp171_[_tmp172_ + 1];
					_tmp174_ = infodir;
					_tmp175_ = dev_node;
					_tmp176_ = string_replace (_tmp175_, "/", "_");
					_tmp177_ = _tmp176_;
					_tmp178_ = g_build_filename (_tmp174_, _tmp177_, NULL);
					_tmp179_ = _tmp178_;
					_tmp180_ = symlink (_tmp173_, _tmp179_);
					_vala_assert (_tmp180_ == 0, "FileUtils.symlink(attributes[i+1],                                          Path.build_filename(infodir, dev_node.replace(\"/\", \"_\"))) == 0");
					_g_free0 (_tmp179_);
					_g_free0 (_tmp177_);
					_tmp182_ = dev_path;
					_tmp183_ = string_contains (_tmp182_, "/block/");
					if (_tmp183_) {
						_tmp181_ = "block";
					} else {
						_tmp181_ = "char";
					}
					_tmp184_ = self->priv->sys_dir;
					_tmp185_ = g_build_filename (_tmp184_, "dev", _tmp181_, NULL);
					sysdev_dir = _tmp185_;
					_tmp186_ = sysdev_dir;
					_tmp187_ = g_mkdir_with_parents (_tmp186_, 0755);
					if (_tmp187_ != 0) {
						...
						_tmp188_ = sysdev_dir;
						_tmp189_ = errno;
						_tmp190_ = g_strerror (_tmp189_);
						g_error ("umockdev.vala:472: cannot create dir '%s': %s", _tmp188_, _tmp190_);
					}
					_tmp191_ = sysdev_dir;
					_tmp192_ = attributes;
					_tmp192__length1 = _vala_array_length (attributes);
					_tmp193_ = i;
					_tmp194_ = _tmp192_[_tmp193_ + 1];
					_tmp195_ = g_build_filename (_tmp191_, _tmp194_, NULL);
					dest = _tmp195_;
					_tmp196_ = dest;
					_tmp197_ = g_file_test (_tmp196_, G_FILE_TEST_EXISTS);
					if (!_tmp197_) {
						...
						_tmp198_ = dev_path;
						_tmp199_ = string_substring (_tmp198_, (glong) 5, (glong) -1);
						_tmp200_ = _tmp199_;
						_tmp201_ = g_strconcat ("../../", _tmp200_, NULL);
						_tmp202_ = _tmp201_;
						_tmp203_ = dest;
						_tmp204_ = symlink (_tmp202_, _tmp203_);
						_tmp205_ = _tmp204_ < 0;
						_g_free0 (_tmp202_);
						_g_free0 (_tmp200_);
						if (_tmp205_) {
							...
							_tmp207_ = dest;
							_tmp208_ = dev_path;
							_tmp209_ = string_substring (_tmp208_, (glong) 5, (glong) -1);
							_tmp210_ = _tmp209_;
							_tmp211_ = errno;
							_tmp212_ = g_strerror (_tmp211_);
							g_error ("umockdev.vala:476: add_device %s: failed to symlink %s to %s: %s\n", _tmp206_, _tmp207_, _tmp210_, _tmp212_);
							_g_free0 (_tmp210_);
						}
					}
					_g_free0 (dest);
	...
	if ((_tmp213__length1 % 2) != 0) {
		...
		_tmp214_ = attributes;
		_tmp214__length1 = _vala_array_length (attributes);
		_tmp215_ = attributes;
		_tmp215__length1 = _vala_array_length (attributes);
		_tmp216_ = _tmp214_[_tmp215__length1 - 1];
		g_warning ("umockdev.vala:482: add_devicev: Ignoring attribute key '%s' without va" \
"lue", _tmp216_);
	}
	_tmp217_ = umockdev_in_mock_environment ();
	if (_tmp217_) {
		const gchar* _tmp218_ = NULL;
		_tmp218_ = dev_path;
		umockdev_testbed_uevent (self, _tmp218_, "add");
	}
	result = dev_path;
	_g_free0 (props);
...
"subsystem of %s: %s", _tmp15_, _tmp17_);
	...
	_g_free0 (_tmp55_);
	_tmp58_ = subsystem;
	if (g_strcmp0 (_tmp58_, "usb") == 0) {
		_tmp57_ = TRUE;
	} else {
		const gchar* _tmp59_ = NULL;
		_tmp59_ = subsystem;
		_tmp57_ = g_strcmp0 (_tmp59_, "pci") == 0;
	}
	if (_tmp57_) {
		_tmp56_ = TRUE;
	} else {
		...
		_tmp60_ = subsystem;
		_tmp56_ = g_strcmp0 (_tmp60_, "thunderbolt") == 0;
	}
	if (_tmp56_) {
		...
		_tmp62_ = subsystem;
		_tmp63_ = devname;
		_tmp64_ = g_build_filename (_tmp61_, "bus", _tmp62_, "devices", _tmp63_, NULL);
		_tmp65_ = _tmp64_;
		g_unlink (_tmp65_);
		_g_free0 (_tmp65_);
		_tmp66_ = self->priv->sys_dir;
		_tmp67_ = subsystem;
		_tmp68_ = g_build_filename (_tmp66_, "bus", _tmp67_, "devices", NULL);
		_tmp69_ = _tmp68_;
		g_rmdir (_tmp69_);
		_g_free0 (_tmp69_);
		_tmp70_ = self->priv->sys_dir;
		_tmp71_ = subsystem;
		_tmp72_ = g_build_filename (_tmp70_, "bus", _tmp71_, NULL);
		_tmp73_ = _tmp72_;
		g_rmdir (_tmp73_);
		_g_free0 (_tmp73_);
	}
	_tmp74_ = real_path;
	umockdev_remove_dir (_tmp74_, TRUE);
	_g_free0 (subsystem);
	_g_free0 (devname);
	_g_free0 (real_path);
}
...
gboolean umockdev_testbed_add_from_string (UMockdevTestbed* self, const gchar* data, GError** error) {
	...
	g_return_val_if_fail (data != NULL, FALSE);
	{
		...
		if (_tmp0_ == NULL) {
			...
			self->priv->re_record_val = _tmp3_;
			_g_regex_unref0 (_tmp1_);
		}
		_tmp4_ = self->priv->re_record_keyval;
		if (_tmp4_ == NULL) {
			GRegex* _tmp5_ = NULL;
			GRegex* _tmp6_ = NULL;
			GRegex* _tmp7_ = NULL;
			_tmp6_ = g_regex_new ("^([EAHL]): ([^=\n]+)=(.*)(?>\n|$)", 0, 0, &_inner_error_);
			_tmp5_ = _tmp6_;
			if (G_UNLIKELY (_inner_error_ != NULL)) {
				if (_inner_error_->domain == G_REGEX_ERROR) {
					goto __catch7_g_regex_error;
			...
			self->priv->re_record_keyval = _tmp7_;
			_g_regex_unref0 (_tmp5_);
		}
		_tmp8_ = self->priv->re_record_optval;
		if (_tmp8_ == NULL) {
			GRegex* _tmp9_ = NULL;
			GRegex* _tmp10_ = NULL;
			GRegex* _tmp11_ = NULL;
			_tmp10_ = g_regex_new ("^([N]): ([^=\n]+)(?>=([0-9A-F]+))?(?>\n|$)", 0, 0, &_inner_error_);
			_tmp9_ = _tmp10_;
			if (G_UNLIKELY (_inner_error_ != NULL)) {
				if (_inner_error_->domain == G_REGEX_ERROR) {
					goto __catch7_g_regex_error;
			...
			self->priv->re_record_optval = _tmp11_;
	...
	__catch7_g_regex_error:
	{
		...
		_inner_error_ = NULL;
		_tmp12_ = e->message;
		g_error ("umockdev.vala:657: Internal error, cannot create regex: %s", _tmp12_);
		_g_error_free0 (e);
	}
	__finally7:
	if (G_UNLIKELY (_inner_error_ != NULL)) {
	...
	while (TRUE) {
		...
		_tmp18_ = cur_data;
		_tmp19_ = umockdev_testbed_add_dev_from_string (self, _tmp18_, &_inner_error_);
		_tmp17_ = _tmp19_;
		if (G_UNLIKELY (_inner_error_ != NULL)) {
			if (_inner_error_->domain == UMOCKDEV_ERROR) {
				g_propagate_error (error, _inner_error_);
...
gboolean umockdev_testbed_load_ioctl (UMockdevTestbed* self, const gchar* dev, const gchar* recordfile, GError** error) {
	...
	if (_tmp3_) {
		...
		__catch8_g_spawn_error:
		{
			...
			_tmp20_ = recordfile;
			_tmp21_ = e->message;
			g_error ("umockdev.vala:745: Cannot call xz to decompress %s: %s", _tmp20_, _tmp21_);
			_g_error_free0 (e);
		}
		__finally8:
		if (G_UNLIKELY (_inner_error_ != NULL)) {
		...
		_tmp28_ = g_data_input_stream_new ((GInputStream*) _tmp22_);
		_g_object_unref0 (recording);
		recording = _tmp28_;
	...
	if (_tmp29_ == NULL) {
		...
		while (TRUE) {
			...
			_tmp37_ = recording;
			_tmp38_ = g_data_input_stream_read_line (_tmp37_, NULL, NULL, &_inner_error_);
			_tmp36_ = _tmp38_;
			if (G_UNLIKELY (_inner_error_ != NULL)) {
				g_propagate_error (error, _inner_error_);
				_g_free0 (line);
		...
		if (_tmp40_ == NULL) {
			...
			_tmp41_ = recordfile;
			g_error ("umockdev.vala:760: ioctl recording file %s has no non-comment content", _tmp41_);
		}
...
gboolean umockdev_testbed_load_script (UMockdevTestbed* self, const gchar* dev, const gchar* recordfile, GError** error) {
	...
	gint _tmp37_ = 0;
	gint _tmp38_ = 0;
	UMockdevScriptRunner* _tmp44_ = NULL;
	const gchar* _tmp45_ = NULL;
	const gchar* _tmp46_ = NULL;
	gint _tmp47_ = 0;
	UMockdevScriptRunner* _tmp48_ = NULL;
	GHashTable* _tmp49_ = NULL;
	const gchar* _tmp50_ = NULL;
	gchar* _tmp51_ = NULL;
	UMockdevScriptRunner* _tmp52_ = NULL;
	GError * _inner_error_ = NULL;
	g_return_val_if_fail (self != NULL, FALSE);
	g_return_val_if_fail (recordfile != NULL, FALSE);
	_tmp0_ = dev;
	...
	if (_tmp2_ == NULL) {
		...
		_tmp9_ = g_data_input_stream_new ((GInputStream*) _tmp3_);
		recording = _tmp9_;
		_tmp10_ = recording;
		...
		if (_tmp20_ == NULL) {
			const gchar* _tmp21_ = NULL;
			_tmp21_ = recordfile;
			g_error ("umockdev.vala:806: script recording %s has no non-comment content", _tmp21_);
		}
...
"o d 0 header", _tmp30_);
	...
	_tmp34_ = owned_dev;
	_tmp35_ = g_hash_table_contains (_tmp33_, _tmp34_);
	_vala_assert (!_tmp35_, "!this.dev_script_runner.contains (owned_dev)");
	_tmp36_ = owned_dev;
	_tmp37_ = umockdev_testbed_get_dev_fd (self, _tmp36_);
	fd = _tmp37_;
	_tmp38_ = fd;
	if (_tmp38_ < 0) {
		const gchar* _tmp39_ = NULL;
		...
		GError* _tmp43_ = NULL;
		_tmp39_ = owned_dev;
		_tmp40_ = g_strconcat (_tmp39_, " is not a device suitable for scripts", NULL);
		_tmp41_ = _tmp40_;
		_tmp42_ = g_error_new_literal (G_FILE_ERROR, G_FILE_ERROR_INVAL, _tmp41_);
		_tmp43_ = _tmp42_;
		_g_free0 (_tmp41_);
	...
	_tmp46_ = recordfile;
	_tmp47_ = fd;
	_tmp48_ = umockdev_script_runner_new (_tmp45_, _tmp46_, _tmp47_, &_inner_error_);
	_tmp44_ = _tmp48_;
	if (G_UNLIKELY (_inner_error_ != NULL)) {
		g_propagate_error (error, _inner_error_);
		_g_free0 (owned_dev);
	...
	_tmp49_ = self->priv->dev_script_runner;
	_tmp50_ = owned_dev;
	_tmp51_ = g_strdup (_tmp50_);
	_tmp52_ = _tmp44_;
	_tmp44_ = NULL;
	g_hash_table_insert (_tmp49_, _tmp51_, _tmp52_);
	result = TRUE;
	_umockdev_script_runner_unref0 (_tmp44_);
	_g_free0 (owned_dev);
	return result;
}
...
gboolean umockdev_testbed_load_evemu_events (UMockdevTestbed* self, const gchar* dev, const gchar* eventsfile, GError** error) {
	...
	_tmp8_ = g_regex_new ("^E: ([0-9]+)\\.([0-9]+) +([0-9a-fA-F]+) +([0-9a-fA-F]+) +(-?[0-9]+) *#" \
"?", 0, 0, &_inner_error_);
	event_re = _tmp8_;
	if (G_UNLIKELY (_inner_error_ != NULL)) {
		g_propagate_error (error, _inner_error_);
	...
	_tmp10_ = g_file_open_tmp ("evemu.XXXXXX.script", &_tmp9_, &_inner_error_);
	...
	while (TRUE) {
		...
		if (!_tmp26_) {
			...
			if (!_tmp28_) {
				...
				_tmp29_ = eventsfile;
				_tmp30_ = line;
				g_warning ("umockdev.vala:909: Ignoring invalid line in %s: %s", _tmp29_, _tmp30_);
			}
			_g_free0 (_tmp11_);
			continue;
		}
		...
		_tmp32_ = g_match_info_fetch (_tmp31_, 1);
		_tmp33_ = _tmp32_;
		_tmp34_ = uint64_parse (_tmp33_);
		_tmp35_ = (time_t) _tmp34_;
		_g_free0 (_tmp33_);
		ev_sec = _tmp35_;
		_tmp36_ = match;
		_tmp37_ = g_match_info_fetch (_tmp36_, 2);
		_tmp38_ = _tmp37_;
		...
		if (_tmp41_) {
			...
			time_t _tmp42_ = 0;
			struct input_event _tmp43_ = {0};
			struct timeval _tmp44_ = {0};
			time_t _tmp45_ = 0;
			time_t _tmp46_ = 0;
			struct input_event _tmp47_ = {0};
			struct timeval _tmp48_ = {0};
			glong _tmp49_ = 0L;
			gint _tmp50_ = 0;
			_tmp42_ = ev_sec;
			_tmp43_ = ev;
		...
		ev.value = (gint32) _tmp64_;
		_g_free0 (_tmp63_);
		_tmp65_ = g_new0 (guint8, sizeof (struct input_event));
		ev_data = _tmp65_;
		...
		memcpy (_tmp66_, &ev, (gsize) _tmp67__length1);
		_tmp68_ = delay;
		_tmp69_ = g_strdup_printf ("%i", _tmp68_);
		_tmp70_ = _tmp69_;
		_tmp71_ = g_strconcat ("r ", _tmp70_, NULL);
		_tmp72_ = _tmp71_;
		_tmp73_ = g_strconcat (_tmp72_, " ", NULL);
		_tmp74_ = _tmp73_;
		_tmp75_ = ev_data;
		_tmp75__length1 = ev_data_length1;
		_tmp76_ = umockdev_script_runner_encode (_tmp75_, _tmp75__length1);
		_tmp77_ = _tmp76_;
		...
		_tmp86_ = _tmp85_;
		write (_tmp82_, _tmp83_, (gsize) _tmp86_);
...
"o '# device' header", _tmp92_);
	...
	_tmp95_ = owned_dev;
	_tmp96_ = script_file;
	_tmp97_ = umockdev_testbed_load_script (self, _tmp95_, _tmp96_, &_inner_error_);
	ret = _tmp97_;
	if (G_UNLIKELY (_inner_error_ != NULL)) {
		g_propagate_error (error, _inner_error_);
		_g_free0 (owned_dev);
				...
				s = s_collection[s_it];
				{
					...
					if (_tmp33_ > 0) {
						...
						sock_path = NULL;
						{
							...
							GSocketAddress* _tmp44_ = NULL;
							GUnixSocketAddress* _tmp45_ = NULL;
							const gchar* _tmp46_ = NULL;
							const gchar* _tmp47_ = NULL;
							gchar* _tmp48_ = NULL;
							gchar* script = NULL;
							...
							gint _tmp59_ = 0;
							gchar* _tmp60_ = NULL;
							UMockdevScriptRunner* _tmp61_ = NULL;
							const gchar* _tmp62_ = NULL;
							const gchar* _tmp63_ = NULL;
							gint _tmp64_ = 0;
							UMockdevScriptRunner* _tmp65_ = NULL;
							GHashTable* _tmp66_ = NULL;
							const gchar* _tmp67_ = NULL;
							gchar* _tmp68_ = NULL;
							UMockdevScriptRunner* _tmp69_ = NULL;
							_tmp42_ = s;
							...
							_tmp44_ = _tmp41_;
							_tmp41_ = NULL;
							_tmp45_ = G_TYPE_CHECK_INSTANCE_CAST (_tmp44_, G_TYPE_UNIX_SOCKET_ADDRESS, GUnixSocketAddress);
							_tmp46_ = g_unix_socket_address_get_path (_tmp45_);
							_tmp47_ = _tmp46_;
							_tmp48_ = g_strdup (_tmp47_);
							_g_free0 (sock_path);
							sock_path = _tmp48_;
							_g_object_unref0 (_tmp45_);
							_tmp49_ = self->priv->socket_scriptfile;
							_tmp50_ = sock_path;
							_tmp51_ = g_hash_table_lookup (_tmp49_, _tmp50_);
							_tmp52_ = g_strdup ((const gchar*) _tmp51_);
							script = _tmp52_;
							_tmp53_ = s;
							_tmp54_ = g_socket_get_fd (_tmp53_);
							_tmp55_ = _tmp54_;
							_tmp56_ = sock_path;
							_tmp57_ = script;
...
"ocket fd %i, path %s, script %s", _tmp55_, _tmp56_, _tmp57_);
							_tmp58_ = sock_path;
							_tmp59_ = fd;
							_tmp60_ = g_strdup_printf ("%s%i", _tmp58_, _tmp59_);
							key = _tmp60_;
							_tmp62_ = key;
							_tmp63_ = script;
							_tmp64_ = fd;
							_tmp65_ = umockdev_script_runner_new (_tmp62_, _tmp63_, _tmp64_, &_inner_error_);
							_tmp61_ = _tmp65_;
							if (G_UNLIKELY (_inner_error_ != NULL)) {
								_g_free0 (key);
								_g_free0 (script);
							...
							_tmp69_ = _tmp61_;
							_tmp61_ = NULL;
							g_hash_table_insert (_tmp66_, _tmp68_, _tmp69_);
							_umockdev_script_runner_unref0 (_tmp61_);
							_g_free0 (key);
							_g_free0 (script);
							_g_object_unref0 (_tmp41_);
						}
